...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentDTO makePayment(PaymentDTO dto) {
        ...
        MaintenanceBill bill = billRepository.findByBillIdAndSociety_SocietyId(dto.getBillId(), societyId)
            .orElseThrow(() -> new ResourceNotFoundException("Bill not found: " + dto.getBillId()));
        if (paymentRepository.findByBill_BillIdAndSociety_SocietyId(dto.getBillId(), societyId).isPresent())
        ...
        Payment payment = Payment.builder().bill(bill).amount(bill.getAmount())
            ...
            .transactionId(txnId).status(Payment.PaymentStatus.SUCCESS).society(bill.getSociety()).build();
    ...
    @Override public Page<PaymentDTO> getAllPayments(Pageable pageable) {
        return paymentRepository.findBySociety_SocietyId(securityUtils.getCurrentSocietyId(), pageable).map(this::toDTO);
    }
    @Override public Page<PaymentDTO> getPaymentsByResident(Long residentId, Pageable pageable) {
        return paymentRepository.findByBill_Resident_ResidentIdAndSociety_SocietyId(
            residentId, securityUtils.getCurrentSocietyId(), pageable).map(this::toDTO);
    }
    @Override public PaymentDTO getPaymentById(Long id) {
        return toDTO(paymentRepository.findByPaymentIdAndSociety_SocietyId(id, securityUtils.getCurrentSocietyId())
